...
public class PacketHandler {
  ...
  public void sendPacket(Packet packet, DatagramSocket socket) {
    ...
    try {
      ...
      System.out.println("\n-- Sending " + packetType + "packet from " +
                         packet.getSourcePort() + " to " +
                         packet.getDestinationPort() + " --");
      datagramPacket = new DatagramPacket(
          bufferWithChecksum, bufferWithChecksum.length,
          InetAddress.getByName(address), packet.getDestinationPort());
  ...
  public Packet receivePacket(DatagramSocket socket) {
    byte[] buffer =
        new byte[ServerThread.packetSize]; // MAKE LENGTH THE MAXIMUM LENGTH A
                                           // PACKET CAN BE
    ...
    try {
      ...
      for (byte b :
           (BigInteger.valueOf(receivedPacket.getCheckSum())).toByteArray()) {
        calculatedCheckSum -= Byte.toUnsignedInt(b);
      }

      // Set if checksum is correct
      receivedPacket.setCheckSumCorrect(calculatedCheckSum ==
                                        receivedPacket.getCheckSum());

    } catch (IOException e) {
      System.out.println("\nTimeout - Resending packet");
      sendPacket(lastPacketSent, socket);
    }
